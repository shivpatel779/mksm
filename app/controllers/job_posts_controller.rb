class JobPostsController < ApplicationController
  ...
  # POST /job_posts.json
  def create
    list_name = params[:job_post][:job_title]
    @board = Board.find(params[:job_post][:board_id])
    @list = Trello::List.create(name: list_name, board_id: @board.trello_board_id)
    params[:job_post][:board_id] = @board.id
    params[:job_post][:trello_list_id] = @list.id
  ...
  private
    ...
    def job_post_params
      params.require(:job_post).permit(:job_title, :job_description, :closing_date, :board_id, :trello_list_id)
    end
end
